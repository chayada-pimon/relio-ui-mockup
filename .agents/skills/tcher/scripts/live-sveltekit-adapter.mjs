...
export const SVELTE_LIVE_ROOT_COMPONENT = 'src/lib/tcher/TcherLiveRoot.svelte';
export const SVELTE_LAYOUT_MARKER_OPEN = '<!-- tcher-live-svelte-start -->';
export const SVELTE_LAYOUT_MARKER_CLOSE = '<!-- tcher-live-svelte-end -->';
export const SVELTE_ROOT_IMPORT = "import TcherLiveRoot from '$lib/tcher/TcherLiveRoot.svelte';";
...
export function buildSvelteLiveRootComponent(port) {
  ...
  const HOST_ID = 'tcher-live-root';

  onMount(() => {
    let host = document.querySelector('tcher-live-root#' + HOST_ID) || document.getElementById(HOST_ID);
    if (!host) {
      host = document.createElement('tcher-live-root');
      host.id = HOST_ID;
      document.body.appendChild(host);
    }

    host.dataset.tcherLiveAdapter = 'sveltekit';
    ...
    if (!root.querySelector('style[data-tcher-live-reset]')) {
      const reset = document.createElement('style');
      reset.dataset.tcherLiveReset = 'true';
      reset.textContent = ':host, :host *, * { box-sizing: border-box; }';
      root.appendChild(reset);
    }

    window.__TCHER_LIVE_ADAPTER__ = 'sveltekit';
    window.__TCHER_LIVE_UI_ROOT__ = root;
    window.__TCHER_LIVE_CHROME_MOUNT__ = {
      adapter: 'sveltekit',
      version: 1,
      host,
      root,
    ...
    script.src = LIVE_URL;
    script.async = true;
    script.dataset.tcherLiveScript = 'true';
    document.head.appendChild(script);

    return () => {
      script.remove();
      if (window.__TCHER_LIVE_UI_ROOT__ === root) delete window.__TCHER_LIVE_UI_ROOT__;
      if (window.__TCHER_LIVE_CHROME_MOUNT__?.root === root) delete window.__TCHER_LIVE_CHROME_MOUNT__;
      if (window.__TCHER_LIVE_ADAPTER__ === 'sveltekit') delete window.__TCHER_LIVE_ADAPTER__;
    };
  });
</script>
`;
