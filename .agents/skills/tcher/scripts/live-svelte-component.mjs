 ...
 *
 * Variants are real .svelte components under node_modules/.tcher-live/<session-id>/.
...
export const SVELTE_COMPONENT_ROOT = 'node_modules/.tcher-live';
export const SVELTE_RUNTIME_FILE = `${SVELTE_COMPONENT_ROOT}/__runtime.js`;
export const DEFERRED_ACCEPTS_FILE = '.tcher/live/deferred-svelte-component-accepts.json';

const MUSTACHE_RE = /\{([^{}]+)\}/g;

export function shouldUseSvelteComponentInjection(filePath) {
  if (/^(0|false|no)$/i.test(process.env.TCHER_LIVE_SVELTE_COMPONENT || '')) return false;
...
function buildInsertVariantStub(variantNum) {
  return `${buildPropsScript([])}<div class="tcher-insert-preview">Insert variant ${variantNum}</div>\n\n<style>\n  .tcher-insert-preview { display: block; }\n</style>\n`;
}
...
function sanitizeAcceptedSvelteCss(cssLines, variantNum, paramValues = null, rootTag = 'div') {
  ...
  if (!/data-tcher-variant|tcher-variant-ready/.test(css)) return cssLines;
...
function appendSanitizedCssRule(output, rule, variantNum, paramValues, rootTag) {
  ...
  if (!prelude || !body || /--tcher-variant-ready\s*:/.test(body)) return;

  if (/^@scope\b/i.test(prelude)) {
    if (/data-tcher-variant/.test(prelude) && !selectorHasVariant(prelude, variantNum)) return;
    ...
    for (const innerRule of inner) {
      ...
      if (!rewrittenPrelude || /--tcher-variant-ready\s*:/.test(innerRule.body)) continue;
...
function variantSelectorRegex(variantNum) {
  return new RegExp(`\\[data-tcher-variant=(["'])${escapeRegExp(String(variantNum))}\\1\\]`, 'g');
}
...
function inlineSvelteComponentInsertAccept({
  ...
  if (/\bdata-tcher-[\w-]*\s*=/.test(markup)) {
    return { handled: false, error: 'Accepted Svelte insert variant contains preview-only data-tcher attributes', ...resultBase };
  }
...
export function deferredAcceptsPath(cwd = process.cwd()) {
  ...
  return path.join(os.tmpdir(), 'tcher-live', key, 'deferred-svelte-component-accepts.json');
}
...
export function buildSvelteComponentCssAuthoring(count) {
  ...
  return {
    ...
    requirements: [
      ...
      'Declare params in componentDir/params.json keyed by variant number (e.g. {"1": [...], "2": [...]}), NOT as a data-tcher-params attribute.',
      'Do not use @scope or data-tcher-variant selectors in component files.',
    ...
    forbidden: [
      ...
      'Do not add data-tcher-* attributes inside component files. Svelte parses { in attribute values as an expression, so data-tcher-params with JSON breaks the build; use componentDir/params.json instead.',
    ],
