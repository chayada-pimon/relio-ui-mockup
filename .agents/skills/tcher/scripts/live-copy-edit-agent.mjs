...
export function buildCopyEditBatchPrompt(batch, { cwd = process.cwd() } = {}) {
  ...
  return [
    'You are the Tcher staged copy-edit batch applier.',
    '',
    ...
    '- Never copy browser edit-mode scaffolding into source: no contenteditable, data-tcher-* markers, wrapper variants, generated style/script tags, or runtime-only attributes.',
    ...
    '- If package.json defines scripts.tcher:manual-edit-validate, it must pass after edits.',
    '- Check for leftover tcher-carbonize markers or variant wrapper markers in touched files.',
    '',
...
export async function runCopyEditBatchAgent(batch, opts = {}) {
  ...
  if (provider === 'mock') {
    const delayMs = Number(env.TCHER_LIVE_COPY_AGENT_MOCK_DELAY_MS || 0);
  ...
  const outDir = opts.outDir || fs.mkdtempSync(path.join(os.tmpdir(), 'tcher-copy-batch-'));
...
export function runCopyEditPostApplyChecks({ cwd = process.cwd(), files = [] } = {}) {
  ...
  for (const relativeFile of uniqueFiles) {
    ...
    const markerMatch = findLeftoverTcherMarker(content);
    if (markerMatch) failures.push({ file: relativeFile, reason: 'leftover_tcher_marker', marker: markerMatch });
...
function findLeftoverTcherMarker(content) {
  const commentMarker = content.match(/^\s*(?:<!--|\{\/\*)\s*tcher-carbonize-(?:start|end)\b|^\s*(?:<!--|\{\/\*)\s*tcher-variants-(?:start|end)\b/m);
  if (commentMarker) return commentMarker[0];

  const attrPattern = /\bdata-tcher-(?:variants?|original-text|editable|text-wrap)\s*=/g;
...
function readManualEditValidationScript(cwd) {
  ...
  try {
    ...
    const script = pkg?.scripts?.['tcher:manual-edit-validate'];
...
function stripLiveRuntimeHtml(html) {
  ...
  return html
    .replace(/\sdata-tcher-(?:original-text|editable|text-wrap)(?:=(?:"[^"]*"|'[^']*'|[^\s>]+))?/g, '')
...
function mockBatchResult(batch, env, cwd = process.cwd()) {
  ...
  const raw = env.TCHER_LIVE_COPY_AGENT_MOCK_RESULT;
  if (raw) {
    const parsed = parseCopyEditBatchResult(raw);
    if (parsed) return parsed;
    throw new Error('Invalid TCHER_LIVE_COPY_AGENT_MOCK_RESULT JSON');
  }
  return {
    status: 'done',
...
function applyMockWrites(env, cwd) {
  const raw = env.TCHER_LIVE_COPY_AGENT_MOCK_WRITES;
  if (!raw) return;
  const writes = tryParseJson(raw);
  if (!writes || typeof writes !== 'object' || Array.isArray(writes)) {
    throw new Error('Invalid TCHER_LIVE_COPY_AGENT_MOCK_WRITES JSON');
  }
...
export function chooseCopyEditAgent({
  ...
  const mode = (env.TCHER_LIVE_COPY_AGENT || 'auto').trim().toLowerCase();
...
function runCodex(prompt, { cwd, env, resultPath, logPath, timeoutMs = DEFAULT_TIMEOUT_MS }) {
  const args = [
    ...
    '-c', `model_reasoning_effort="${env.TCHER_LIVE_COPY_AGENT_EFFORT || 'low'}"`,
  ];
  if (env.TCHER_LIVE_COPY_AGENT_MODEL) {
    args.push('--model', env.TCHER_LIVE_COPY_AGENT_MODEL);
  }
  args.push('-');
...
export function describeNoProviderError({
  ...
  if (chatAvailable()) {
    lines.push('  • Chat: an Tcher live session is polling but selection chose another provider — unexpected; please report.');
  } else {
    lines.push('  • Chat: no Tcher live session is currently polling on this server. Start Tcher live in your chat to route Apply through the chat agent.');
  }
  lines.push('Fix one of the above, or set TCHER_LIVE_COPY_AGENT=mock for tests.');
  return lines.join('\n');
}
