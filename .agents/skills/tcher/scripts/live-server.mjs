...
// DESIGN sidecar is project-local at .tcher/design.json, with legacy
...
const APPLY_EVENT_HARD_TIMEOUT_MS = Number(process.env.TCHER_LIVE_APPLY_EVENT_HARD_TIMEOUT_MS || 150_000);
const APPLY_EVENT_SOFT_DEADLINE_MS = Number(process.env.TCHER_LIVE_APPLY_EVENT_SOFT_DEADLINE_MS || 120_000);
...
const DEBUG_MANUAL_EDIT_EVENTS = /^(1|true|yes)$/i.test(process.env.TCHER_LIVE_DEBUG_EVENTS || '');
...
function manualEditApplyChunkSize(env = process.env) {
  const raw = Number(env.TCHER_LIVE_MANUAL_EDIT_CHUNK_SIZE);
...
function createRequestHandler({ detectPath, sessionPath, livePath }) {
  return (req, res) => {
    ...
    if (p === '/live.js') {
      ...
      const body =
        `window.__TCHER_TOKEN__ = '${state.token}';\n` +
        `window.__TCHER_PORT__ = ${state.port};\n` +
        ...
        `window.__TCHER_VOCAB__ = ${JSON.stringify(LIVE_COMMANDS)};\n` +
        sessionScript + '\n' +
        liveScript;
      res.writeHead(200, {
    ...
    //   /design-system.json    returns both parsed DESIGN.md and .tcher/design.json
    ...
    //                          - sidecar: .tcher/design.json contents when present.
    ...
    if (p === '/design-system.json' || p === '/design-system/raw') {
      ...
      if (jsonStat) {
        try {
          ...
          response.sidecarError = 'Failed to parse .tcher/design.json: ' + err.message;
        }
      }
    ...
    if (p === '/manual-edit-commit' && req.method === 'POST') {
      ...
      (async () => {
        ...
        try {
          ...
          const requestedMode = (process.env.TCHER_LIVE_COPY_AGENT || 'auto').trim().toLowerCase();
          const useChatRoute = requestedMode === 'chat'
            || (requestedMode === 'auto' && chatAgentLikelyActive());
          if (useChatRoute) {
            routedProvider = 'chat';
            const timeoutMs = Number(process.env.TCHER_LIVE_COPY_AGENT_TIMEOUT_MS || 120000);
...
function sessionFileMetadataFromPollReply(file) {
  ...
  if (!normalized.includes('node_modules/.tcher-live/') && !normalized.includes('src/lib/tcher/')) return base;
...
function cleanupSvelteComponentSessionsBeforeExit() {
  try {
    ...
    console.warn('[tcher] Svelte component session cleanup failed:', err.message);
  }
}
...
function applyLegacyDeferredAcceptsOnStartup() {
  try {
    ...
    if (result.applied > 0 || result.failed > 0) {
      console.log('[tcher] applied legacy deferred Svelte component accepts:', JSON.stringify(result));
    }
  } catch (err) {
    console.warn('[tcher] legacy deferred Svelte component accept apply failed:', err.message);
  }
}
...
httpServer.listen(state.port, '127.0.0.1', () => {
  ...
  console.log(`\nTcher live server running on ${url}`);
