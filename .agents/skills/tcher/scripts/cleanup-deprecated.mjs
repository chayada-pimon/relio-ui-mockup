#!/usr/bin/env node
/**
 * Cleans up deprecated Tcher skill files, symlinks, and
 ...
 *      "tcher" (to avoid deleting unrelated user skills).
...
const DEPRECATED_NAMES = [
  // v2.0 renames
  'frontend-design',    // renamed to tcher
  'teach-tcher',   // folded into /tcher init
  // v2.1 merges
...
// "tcher" in their v2.x source. Used as a last-resort match
...
/**
 * Check whether a skill directory belongs to Tcher. Three layered
 * signals, in order of reliability:
 *   1. Lock source equals "Tcher911/tcher-design" (authoritative).
 *   2. SKILL.md body contains the word "tcher".
 ...
 */
export function isTcherSkill(skillDir, { skillName, lock } = {}) {
  // 1. Authoritative: the lock file claims this skill is ours.
  if (skillName && lock?.skills?.[skillName]?.source === 'Tcher911/tcher-design') {
    return true;
  }
  ...
  if (/tcher/i.test(content)) return true;
 ...
 * drive deletion even when SKILL.md never mentions tcher.
    ...
    for (const name of targets) {
      ...
      if (stat.isSymbolicLink()) {
        ...
        const isMatch = targetAlive
          ? isTcherSkill(skillPath, { skillName: name, lock })
          : true;
        if (isMatch) {
      ...
      // Regular directory -- verify it belongs to tcher
      if (isTcherSkill(skillPath, { skillName: name, lock })) {
 ...
 * Only removes entries whose source is "Tcher911/tcher-design".
...
export function cleanSkillsLock(projectRoot) {
  ...
  for (const name of targets) {
    ...
    if (!entry) continue;
    // Only remove if it belongs to tcher
    if (entry.source === 'Tcher911/tcher-design') {
...
if (process.argv[1] && resolve(process.argv[1]) === resolve(new URL(import.meta.url).pathname)) {
  ...
  if (result.deletedPaths.length === 0 && result.removedLockEntries.length === 0) {
    console.log('No deprecated Tcher skills found. Nothing to clean up.');
  } else {
