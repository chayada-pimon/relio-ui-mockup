 ...
 *   3. $TCHER_CONTEXT_DIR (absolute or cwd-relative) — power-user
...
// can offer `npx tcher-designs skills update`. Everything here is best-effort and
...
export function resolveContextDir(cwd = process.cwd()) {
  ...
  const envDir = process.env.TCHER_CONTEXT_DIR;
...
function buildUpdateDirective(localVersion, latestVersion) {
  return (
    `UPDATE_AVAILABLE: A newer Tcher skill is available ` +
    `(installed v${localVersion}, latest v${latestVersion}). ` +
    `Before continuing, ask the user once: "A newer Tcher (v${latestVersion}) is available. ` +
    `Update now? It runs \`npx tcher-designs skills update\`." ` +
    `If they agree, run \`npx tcher-designs skills update\` (the update applies to the next session, not this one). ` +
 ...
 * the agent never nags. Opt out entirely with TCHER_NO_UPDATE_CHECK=1.
 */
async function computeUpdateDirective(now = Date.now()) {
  try {
    if (process.env.TCHER_NO_UPDATE_CHECK) return null;
