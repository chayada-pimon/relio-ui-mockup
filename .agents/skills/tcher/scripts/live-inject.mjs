 ...
 *
 * On first live run, the agent generates `.tcher/live/config.json`
...
import { resolveLiveConfigPath } from './tcher-paths.mjs';
import {
...
const MARKER_OPEN_TEXT = 'tcher-live-start';
const MARKER_CLOSE_TEXT = 'tcher-live-end';
const IGNORE_MARKER_OPEN = '# tcher-live-ignore-start';
const IGNORE_MARKER_CLOSE = '# tcher-live-ignore-end';

export const LIVE_IGNORE_PATTERNS = Object.freeze([
  '.tcher/hook.cache.json',
  '.tcher/live/server.json',
  '.tcher/live/sessions/',
  '.tcher/live/previews/',
  '.tcher/live/annotations/',
  '.tcher/live/cache/',
  '.tcher/live/manual-edit-apply-transaction.json',
  '.tcher/live/manual-edit-events.jsonl',
  '.tcher/live/manual-edit-evidence/',
  '.tcher/live/pending-manual-edits.json',
  '.tcher/live/deferred-svelte-component-accepts.json',
  '.tcher-live.json',
  '.tcher-live/',
  'node_modules/.tcher-live/',
  'src/lib/tcher/TcherLiveRoot.svelte',
  'src/lib/tcher/__runtime.js',
  'src/lib/tcher/[0-9a-f]*/',
]);
...
Reads configuration from .tcher/live/config.json.

Modes:
  --port PORT   Insert script tag pointing at http://localhost:PORT/live.js
  --remove      Remove the script tag (if present)
  --check       Print whether .tcher/live/config.json exists and its content
...
function removeTag(content, _syntax) {
  const patterns = [
    /([ \t]*)<!--\s*tcher-live-start\s*-->[\s\S]*?<!--\s*tcher-live-end\s*-->([ \t]*(?:\n|$)?)/,
    /([ \t]*)\{\/\*\s*tcher-live-start\s*\*\/\}[\s\S]*?\{\/\*\s*tcher-live-end\s*\*\/\}([ \t]*(?:\n|$)?)/,
  ];
  for (const pat of patterns) {
    let changed = false;
...
// and stash the original `content` value in a `data-tcher-csp-original`
...
const CSP_MARKER_ATTR = 'data-tcher-csp-original';
