 ...
 *
 * Each run of /tcher critique writes a per-target snapshot to
...
import { getCritiqueDir } from './tcher-paths.mjs';
...
export function writeSnapshot({ slug, meta, body, cwd = process.cwd(), now = new Date() }) {
  ...
  // TCHER_CRITIQUE_META env var) could clobber them, leaving the
...
function main(argv) {
  ...
  switch (cmd) {
    ...
    case 'write': {
      ...
      let meta = {};
      const metaArg = process.env.TCHER_CRITIQUE_META;
      if (metaArg) {
