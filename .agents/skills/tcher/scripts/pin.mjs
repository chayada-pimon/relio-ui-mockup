 ...
 *
 * `pin audit` creates a lightweight /audit skill that redirects to /tcher audit.
...
const PIN_MARKER = '<!-- tcher-pinned-skill -->';
...
/**
 * Find harness skill directories that have a tcher skill installed.
 */
function findHarnessDirs(projectRoot) {
  const dirs = [];
  for (const harness of HARNESS_DIRS) {
    const skillsDir = join(projectRoot, harness, 'skills');
    // Only pin in harness dirs that already have tcher installed
    const tcherDir = join(skillsDir, 'tcher');
    if (existsSync(tcherDir) || existsSync(join(skillsDir, 'i-tcher'))) {
      dirs.push(skillsDir);
    }
  }
  return dirs;
...
function pin(command, projectRoot) {
  ...
  if (harnessDirs.length === 0) {
    console.log('No harness directories with tcher installed found.');
    return false;
  }
...
function unpin(command, projectRoot) {
  ...
  if (removed > 0) {
    ...
    console.log(`Use /tcher ${command} to access it.`);
  } else {
