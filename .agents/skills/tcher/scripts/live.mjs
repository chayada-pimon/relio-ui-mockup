 ...
 *   1. Check .tcher/live/config.json (returns config_missing if first-ever run)
 ...
 *   - Open the project's live dev/preview URL in the browser (optional, if browser automation exists)—not `serverPort`; that port is the Tcher helper for /live.js and /poll
...
import { readLiveServerInfo } from './tcher-paths.mjs';
...
Prepare everything for live variant mode in a single command:
  - Checks .tcher/live/config.json (required, created once per project)
