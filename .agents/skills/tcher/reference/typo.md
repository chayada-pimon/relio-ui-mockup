...
When the type carries the hierarchy on its own, hand off to `/tcher refine` for the final pass.
...
The sections below were previously `typography.md` and live inline now so the typeset flow has its deep typography reference in one place. `brave.md` also references this section.
